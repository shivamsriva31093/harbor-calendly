...
public class UserServiceVertxEBProxy implements UserService {
  ...
  @Override
  public Future<OperationResponse> createUser(JsonObject request){
  ...
  @Override
  public Future<OperationResponse> updateUser(String userId, JsonObject request){
    if (closed) return io.vertx.core.Future.failedFuture("Proxy is closed");
    JsonObject _json = new JsonObject();
  ...
  @Override
  public Future<OperationResponse> getUser(String userId){
    if (closed) return io.vertx.core.Future.failedFuture("Proxy is closed");
    JsonObject _json = new JsonObject();
    _json.put("userId", userId);
  ...
  @Override
  public Future<OperationResponse> deleteUser(String userId){
    if (closed) return io.vertx.core.Future.failedFuture("Proxy is closed");
    JsonObject _json = new JsonObject();
    _json.put("userId", userId);
  ...
  @Override
  public Future<OperationResponse> getUserTimezone(String userId){
    if (closed) return io.vertx.core.Future.failedFuture("Proxy is closed");
    JsonObject _json = new JsonObject();
    _json.put("userId", userId);

    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "getUserTimezone");
    _deliveryOptions.getHeaders().set("action", "getUserTimezone");
    return _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions).map(msg -> {
      return msg.body() != null ? new io.harbor.calendly.repositories.models.OperationResponse((JsonObject)msg.body()) : null;
    });
  }
