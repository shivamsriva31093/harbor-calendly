...
public class UserServiceVertxProxyHandler extends ProxyHandler {
  ...
  public void handle(Message<JsonObject> msg) {
    try{
      ...
      switch (action) {
        case "createUser": {
          service.createUser((io.vertx.core.json.JsonObject)json.getValue("request")).onComplete(res -> {
                        if (res.failed()) {
        ...
        case "deleteUser": {
          service.deleteUser((java.lang.String)json.getValue("userId")).onComplete(res -> {
                        if (res.failed()) {
                          HelperUtils.manageFailure(msg, res.cause(), includeDebugInfo);
                        } else {
                          msg.reply(res.result() != null ? res.result().toJson() : null);
        ...
        case "getUserTimezone": {
          service.getUserTimezone((java.lang.String)json.getValue("userId")).onComplete(res -> {
                        if (res.failed()) {
                          HelperUtils.manageFailure(msg, res.cause(), includeDebugInfo);
                        } else {
                          msg.reply(res.result() != null ? res.result().toJson() : null);
